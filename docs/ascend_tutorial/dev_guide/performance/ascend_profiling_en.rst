...
-  global_profiler: Control the ranks and mode of profiling
   ...
   -  steps: This parameter can be set as a list containing collection steps, for example [2, 4], which means collecting step 2 and step 4. If set to null, no collection occurs.
...
.. code:: bash
      ...
      actor_rollout_ref.rollout.profiler.ranks="[0]" # Only collect rank 0
      # Optional: lightweight collection of inference data, collecting by response token interval; when start/stop are not set, the entire rollout phase is collected
...
.. code:: bash
    ...
    export PROFILE_STEP=2
    export ROLLOUT_PROFILE=true
    export LOGPROB_PROFILE=false
    export UPDATE_PROFILE=true
    export WITH_MODULES=false
    export WITH_STACK=false
    export WITH_MEMORY=false
    export WITH_SHAPE=true
...
- **Reference Version**:vLLM v0.18.0, vLLM-Ascend v0.18.1rc1
...
.. code-block:: diff
  ...
  +                   with_modules=os.environ.get('WITH_MODULES', "false") == "false",  # Collect call stack
  +                   profile_memory=os.environ.get('WITH_MEMORY', "false") == "true",  # Collect memory
...
Micro-Batch scheduling in the Megatron backend is managed internally by Megatron pipeline-parallel
``forward_backward_func`` and does not currently support fine-grained collection at the Micro-Batch level
through simple code instrumentation. It is recommended to use the global profiler configuration for collection.
